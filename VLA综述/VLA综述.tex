...
从表1可以看出，VLA模型的参数规模从35M到562B不等，视觉编码器主要使用ViT，语言模型主要使用Transformer系列，动作表示从连续值发展到文本令牌、扩散过程、流匹配等多种形式。
...
从表2可以看出，VLA模型的训练方法从行为克隆发展到联合训练、扩散训练、流匹配训练等多种方法，微调方法从无发展到LoRA、强化学习等，数据需求从高发展到低。
...
从表3可以看出，VLA模型的推理速度从3Hz发展到117.7Hz，零样本泛化能力从无发展到有，性能不断提升。
